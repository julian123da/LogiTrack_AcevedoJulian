const API_BASE = (() => {
  ...
  return `${location.protocol === 'file:' ? 'http:' : location.protocol}//${host}:8080`;
})();
...
async function loadAllData(showReloadToast = false) {
  ...
  state.data.usuarios = resultados[0].status === 'fulfilled' ? ensureArray(resultados[0].value) : [];
  state.data.bodegas = resultados[1].status === 'fulfilled' ? ensureArray(resultados[1].value) : [];
  state.data.productos = resultados[2].status === 'fulfilled' ? ensureArray(resultados[2].value) : [];
  state.data.movimientos = resultados[3].status === 'fulfilled' ? ensureArray(resultados[3].value) : [];
  state.data.detalles = resultados[4].status === 'fulfilled' ? ensureArray(resultados[4].value) : [];
  state.data.auditorias = resultados[5].status === 'fulfilled' ? ensureArray(resultados[5].value) : [];
  ...
  if (errores.length) {
    showToast(`Algunos módulos no cargaron: ${errores.join(' | ')}`, 'error');
  } else if (showReloadToast) {
